...
def _plan_body(s: Optional[dict], name: str, vm_ids: List[str], target_ns: str, warm: bool) -> dict:
    s = s or {}
    ns = _ns(s)
    ref = lambda key, default: {"name": s.get(key, default), "namespace": ns}
    return {
        "apiVersion": f"{GROUP}/{VERSION}",
        "kind": "Plan",
        "metadata": {"name": name, "namespace": ns,
                     "labels": {"created-by": "migration-copilot"}},
        "spec": {
            "warm": warm,
            "targetNamespace": target_ns,
            "provider": {"source": ref("source_provider", "vsphere"),
                         "destination": ref("destination_provider", "host")},
            "map": {"network": ref("network_map", "vsphere-network-map"),
...
def _audit(event: str, **fields):
    ...
    log.warning("AUDIT %s %s", event, {"at": dt.datetime.now(dt.timezone.utc).isoformat(), **fields})


# ---------- tools ----------

@tool(permission=ToolPermission.READ_WRITE, expected_credentials=CREDS)
def create_migration_plan(plan_name: str, vm_ids: List[str], target_namespace: str,
                          warm: bool = False, dry_run: bool = True) -> dict:
    """
    ...
    return {"mode": "live" if s else "demo", "saved": saved,
            "vm_count": len(vm_ids), "plan_yaml": yaml.safe_dump(body, sort_keys=False)}


@tool(permission=ToolPermission.READ_WRITE, expected_credentials=CREDS)
def start_migration(plan_name: str, approver_name: str, approval_confirmed: bool,
                    change_ticket: Optional[str] = None) -> dict:
    """
    ...
    audit = {"plan": plan_name, "approved_by": approver_name,
             "change_ticket": change_ticket or "none", "approved_at": stamp}
    if s:
        from kubernetes import client
        body = {
            "apiVersion": f"{GROUP}/{VERSION}", "kind": "Migration",
            "metadata": {"generateName": f"{plan_name}-", "namespace": _ns(s),
...
def migration_status(plan_name: str) -> dict:
    ...
    if not s:
        ...
        return {"mode": "demo", "plan": plan_name,
    ...
    for vm in status.get("migration", {}).get("vms", []):
        ...
        vms.append({"vm": vm.get("name") or vm.get("id"), "phase": vm.get("phase"),
                    "step": current.get("name"), "percent": pct,
...
def list_openshift_vms(namespace: str) -> dict:
    ...
    s = _settings()
    if not s:
        return {"mode": "demo", "namespace": namespace,
                "vms": [{"name": "fin-web-01", "status": "Running"}]}
    from kubernetes import client
    res = client.CustomObjectsApi(_api(s)).list_namespaced_custom_object(
        "kubevirt.io", "v1", namespace, "virtualmachines")
    return {"mode": "live", "namespace": namespace,
            "vms": [{"name": i["metadata"]["name"],
                     "status": i.get("status", {}).get("printableStatus", "Unknown")}
